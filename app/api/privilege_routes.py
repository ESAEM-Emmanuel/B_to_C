...
from app.schemas.privileges_schemas import (
    PrivilegeCreate,
    PrivilegeUpdate,
    PrivilegeSchema,)
...
from app.crud.privilege_crud import (
    research,
    create,
    update,
    delete,
...
get_user_from_token)
router = APIRouter(prefix="/privileges", tags=["Privileges Requests"])

@router.post("/", status_code=status.HTTP_201_CREATED, response_model=PrivilegeSchema)
async def create_route(
    item: PrivilegeCreate, 
    ...
    try:
        ...
        # Retourne le privilegeavec le créateur sérialisé
        return PrivilegeSchema.from_orm(item).copy(update={
            "creator": creator,
            "updator": updator}
        )
...
@router.get("/", response_model=PaginatedResponse[PrivilegeSchema])  # Spécifiez PrivilegeSchema ici
async def research_route(
    ...
    return PaginatedResponse[PrivilegeSchema](  # Utilisez PaginatedResponse[PrivilegeSchema]
        records=serialized,
...
@router.get("/{id}", response_model=PrivilegeSchema)
...
@router.put("/{id}", response_model=PrivilegeSchema)
async def update_route(
    id: str, 
    n_update: PrivilegeUpdate, 
    db: Session = Depends(get_db), 
...
@router.patch("/restore/{id}", response_model=PrivilegeSchema)
