...
from app.schemas.user_roles_schemas import (
    UserRoleCreate,
    UserRoleUpdate,
    UserRoleSchema,)
...
router = APIRouter(prefix="/user_roles", tags=["UserRoles Requests"])

@router.post("/", status_code=status.HTTP_201_CREATED, response_model=UserRoleSchema)
async def create_route(
    ...
    try:
        ...
        # Retourne le UserRole avec le créateur sérialisé
        return UserRoleSchema.from_orm(item).copy(update={
            "creator": creator,
            "updator": updator}
        )
...
@router.get("/", response_model=PaginatedResponse[UserRoleSchema])  # Spécifiez UserRoleSchema ici
async def research_route(
    ...
    serialized = [
        UserRoleSchema(
            ...
            role=RoleSchema.model_validate(item.role),  # Transforme la relation privilege en PrivilegeSchema
            creator=get_user_by_id(db, item.created_by) if item.created_by else None,
            updator=get_user_by_id(db, item.updated_by) if item.updated_by else None
        )
        for item in items
    ...
    return PaginatedResponse[UserRoleSchema](  # Utilisez PaginatedResponse[UserRoleSchema]
        records=serialized,
...
@router.put("/{id}", response_model=UserRoleSchema)
async def update_route(
    id: str, 
    n_update: UserRoleUpdate, 
    db: Session = Depends(get_db), 
...
@router.patch("/restore/{id}", response_model=UserRoleSchema)
