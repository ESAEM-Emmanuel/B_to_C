...
from app.schemas.roles_schemas import (
    RoleCreate,
    RoleUpdate,
    RoleSchema,)
...
get_user_from_token)
router = APIRouter(prefix="/roles", tags=["Roles Requests"])

@router.post("/", status_code=status.HTTP_201_CREATED, response_model=RoleSchema)
async def create_route(
    ...
    try:
        ...
        return RoleSchema.from_orm(item).copy(update={
            "creator": creator,
            "updator": updator}
        )
...
@router.get("/", response_model=PaginatedResponse[RoleSchema])  # Spécifiez RoleSchema ici
async def research_route(
    ...
    return PaginatedResponse[RoleSchema](  # Utilisez PaginatedResponse[RoleSchema]
        records=serialized,
...
async def get_detail(id: str, db: Session = Depends(get_db)):
    ...
    # return RoleSchema.from_orm(item)
...
@router.patch("/restore/{id}", response_model=RoleSchema)
