...
from app.schemas.privilege_roles_schemas import (
    PrivilegeRoleCreate,
    PrivilegeRoleUpdate,
    PrivilegeRoleSchema,)
...
from app.crud.privilege_role_crud import (
    research,
    create,
    update,
    delete,
...
get_user_from_token)
router = APIRouter(prefix="/privilege_roles", tags=["PrivilegeRoles Requests"])


@router.post("/", status_code=status.HTTP_201_CREATED, response_model=PrivilegeRoleSchema)
async def create_route(
    item: PrivilegeRoleCreate, 
    ...
    try:
        ...
        # Retourne le PrivilegeRole avec le créateur sérialisé
        return PrivilegeRoleSchema.from_orm(item).copy(update={
            "creator": creator,
            "updator": updator
        })
...
@router.get("/", response_model=PaginatedResponse[PrivilegeRoleSchema])  # Spécifiez PrivilegeRoleSchema ici
async def research_route(
    ...
    #         role=RoleSchema.model_validate(item.role),  # Transforme la relation role en RoleSchema
    #         privilege=PrivilegeSchema.model_validate(item.privilege), 
    #         creator=get_user_by_id(db, item.created_by) if item.created_by else None,
    #         updator=get_user_by_id(db, item.updated_by) if item.updated_by else None
    #     )
    #     for item in items
    ...
    return PaginatedResponse[PrivilegeRoleSchema](  # Utilisez PaginatedResponse[PrivilegeRoleSchema]
        records=serialized,
...
@router.get("/{id}", response_model=PrivilegeRoleSchema)
...
@router.put("/{id}", response_model=PrivilegeRoleSchema)
async def update_route(
    id: str, 
    n_update: PrivilegeRoleUpdate, 
    db: Session = Depends(get_db), 
    ...
    try:
        ...
        # Retourne le PrivilegeRoleavec le créateur sérialisé
        return PrivilegeRoleSchema.from_orm(item).copy(update={
            "creator": creator,
            "updator": updator}
        )
...
@router.patch("/restore/{id}", response_model=PrivilegeRoleSchema)
