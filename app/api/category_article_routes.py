...
from app.crud.category_article_crud import (
    research,
    create,
    update,
    delete,
...
get_user_from_token)
router = APIRouter(prefix="/category_articles", tags=["Category Articles Requests"])

@router.post("/", status_code=status.HTTP_201_CREATED, response_model=CategoryArticleSchema)
async def create_route(
    item: CategoryArticleCreate, 
    ...
    try:
        ...
        # Retourne le CategoryArticleavec le créateur sérialisé
        return CategoryArticleSchema.from_orm(item).copy(update={
            "creator": creator,
            "updator": updator}
        )
...
@router.get("/", response_model=PaginatedResponse[CategoryArticleSchema])  # Spécifiez CategoryArticleSchema ici
async def research_route(
    ...
    return PaginatedResponse[CategoryArticleSchema](  # Utilisez PaginatedResponse[CategoryArticleSchema]
        records=serialized,
...
@router.get("/{id}", response_model=CategoryArticleSchema)
...
@router.put("/{id}", response_model=CategoryArticleSchema)
async def update_route(
    id: str, 
    n_update: CategoryArticleUpdate, 
    db: Session = Depends(get_db), 
...
@router.patch("/restore/{id}", response_model=CategoryArticleSchema)
