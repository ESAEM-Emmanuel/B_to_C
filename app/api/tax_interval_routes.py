...
from app.schemas.tax_intervals_schemas import (
    TaxIntervalCreate,
    TaxIntervalUpdate,
    TaxIntervalSchema,)
...
from app.crud.tax_interval_crud import (
    research,
    create,
    update,
    delete,
...
get_user_from_token)
router = APIRouter(prefix="/tax_intervals", tags=["Tax Interval Requests"])

@router.post("/", status_code=status.HTTP_201_CREATED, response_model=TaxIntervalSchema)
async def create_route(
    item: TaxIntervalCreate, 
    ...
    try:
        ...
        return TaxIntervalSchema.from_orm(item).copy(update={
            "creator": creator,
            "updator": updator}
        )
...
@router.get("/", response_model=PaginatedResponse[TaxIntervalSchema])
async def research_route(
    min_price: Optional[float] = Query(None),
    min_price_bis: Optional[float] = Query(None),
    min_price_operation: Optional[str] = Query(None),
    max_price: Optional[float] = Query(None),
    max_price_bis: Optional[float] = Query(None),
    max_price_operation: Optional[str] = Query(None),
    daily_rate: Optional[float] = Query(None),
    daily_rate_bis: Optional[float] = Query(None),
    daily_rate_operation: Optional[str] = Query(None),
    ...
    items, total_records = research(
        db=db,
        min_price=min_price,
        min_price_bis=min_price_bis,
        min_price_operation=min_price_operation,
        max_price=max_price,
        max_price_bis=max_price_bis,
        max_price_operation=max_price_operation,
        daily_rate=daily_rate,
        daily_rate_bis=daily_rate_bis,
        daily_rate_operation=daily_rate_operation,
        refnumber=refnumber,
    ...
    return PaginatedResponse[TaxIntervalSchema](
        records=serialized,
...
@router.get("/{id}", response_model=TaxIntervalSchema)
...
@router.put("/{id}", response_model=TaxIntervalSchema)
async def update_route(
    id: str, 
    n_update: TaxIntervalUpdate, 
    db: Session = Depends(get_db), 
...
@router.patch("/restore/{id}", response_model=TaxIntervalSchema)
