...
from app.schemas.countries_schemas import (
    CountryCreate,
    CountryUpdate,
    CountrySchema,)
...
get_user_from_token)
router = APIRouter(prefix="/countries", tags=["Countries Requests"])
...
async def create_route(
    ...
    try:
        ...
        # Retourne le Countryavec le créateur sérialisé
        return CountrySchema.from_orm(item).copy(update={
            "creator": creator,
            "updator": updator}
        )
...
@router.get("/", response_model=PaginatedResponse[CountrySchema])  # Spécifiez CountrySchema ici
async def research_route(
    ...
    return PaginatedResponse[CountrySchema](  # Utilisez PaginatedResponse[CountrySchema]
        records=serialized,
...
@router.patch("/restore/{id}", response_model=CountrySchema)
