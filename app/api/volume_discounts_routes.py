...
from app.schemas.volume_discounts_schemas import (
    VolumeDiscountsCreate,
    VolumeDiscountsUpdate,
    VolumeDiscountsSchema,)
from app.schemas.utils_schemas import (PaginatedResponse, PaginationMetadata)
from app.database import get_db
from typing import Optional, List
from datetime import date, datetime, time
from app.crud.volume_discounts_crud import (
    research,
    create,
    update,
    delete,
...
get_user_from_token)
router = APIRouter(prefix="/volume_discounts", tags=["Volume Discounts Requests"])

@router.post("/", status_code=status.HTTP_201_CREATED, response_model=VolumeDiscountsSchema)
async def create_route(
    item: VolumeDiscountsCreate, 
    ...
    try:
        ...
        return VolumeDiscountsSchema.from_orm(item).copy(update={
            "creator": creator,
            "updator": updator}
        )
...
@router.get("/", response_model=PaginatedResponse[VolumeDiscountsSchema])
async def research_route(
    threshold: Optional[int] = Query(None),
    threshold_bis: Optional[int] = Query(None),
    threshold_operation: Optional[str] = Query(None),
    reduction: Optional[float] = Query(None),
    reduction_bis: Optional[float] = Query(None),
    reduction_operation: Optional[str] = Query(None),
    ...
    items, total_records = research(
        db=db,
        threshold=threshold,
        threshold_bis=threshold_bis,
        threshold_operation=threshold_operation,
        reduction=reduction,
        reduction_bis=reduction_bis,
    ...
    return PaginatedResponse[VolumeDiscountsSchema](
        records=serialized,
...
@router.get("/{id}", response_model=VolumeDiscountsSchema)
...
@router.put("/{id}", response_model=VolumeDiscountsSchema)
async def update_route(
    id: str, 
    n_update: VolumeDiscountsUpdate, 
    db: Session = Depends(get_db), 
...
@router.patch("/restore/{id}", response_model=VolumeDiscountsSchema)
