...
from app.schemas.towns_schemas import (
    TownCreate,
    TownUpdate,
    TownSchema,)
...
from app.crud.town_crud import (
    research,
    create,
    update,
    delete,
...
@router.post("/", status_code=status.HTTP_201_CREATED, response_model=TownSchema)
async def create_route(
    ...
    try:
        ...
        # Retourne le Townavec le créateur sérialisé
        return TownSchema.from_orm(item).copy(update={
            "creator": creator,
            "updator": updator}
        )
...
@router.get("/", response_model=PaginatedResponse[TownSchema])  # Spécifiez TownSchema ici
async def research_route(
    ...
    return PaginatedResponse[TownSchema](  # Utilisez PaginatedResponse[TownSchema]
        records=serialized,
...
@router.get("/{id}", response_model=TownSchema)
...
@router.put("/{id}", response_model=TownSchema)
...
@router.patch("/restore/{id}", response_model=TownSchema)
