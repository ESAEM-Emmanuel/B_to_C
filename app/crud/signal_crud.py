...
# ):
#     # Validation des paramètres
#     validate_parameters(order, created_at_operation, updated_at_operation)
...
#         query = query.filter(Signal.offender_id == offender_id)
#     if description:
#         query = query.filter(Signal.description.ilike(f"%{description}%"))
#     if refnumber:
#         query = query.filter(Signal.refnumber.ilike(f"%{refnumber}%"))
#     if created_by is not None:
#         query = query.filter(Signal.created_by == created_by)
...
#             db.rollback()
#             raise Exception(f"Erreur lors de la création du signal : {str(e)}")
#     else :
#         existing.active = toggle_boolean(existing.active)
#         existing.updated_by = current_user_id
#         try:
#             db.add(existing )# pour ajouter une tuple
#             db.commit() # pour faire l'enregistrement
#             db.refresh(existing)# pour renvoyer le résultat
#         except SQLAlchemyError as e:
#             db.rollback()
#             raise HTTPException(status_code=403, detail="Erreur lors de la mise à jour du signal!")  
