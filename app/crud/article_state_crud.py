...
from app.models.models import ArticleState
from app.schemas.article_states_schemas import ArticleStateCreate, ArticleStateUpdate
...
def get_by_id(db: Session, item_id: str):
    return db.query(ArticleState).filter(ArticleState.id == item_id).first()
...
def research(
    ...
    valid_sort_columns = [column.key for column in ArticleState.__table__.columns]
    if sort_by not in valid_sort_columns:
        raise ValueError(f"Invalid sort_by value: {sort_by}. Valid options are: {valid_sort_columns}")

    # Construction de la requête
    query = db.query(ArticleState)

    # Filtres génériques
    if name:
        query = query.filter(ArticleState.name.ilike(f"%{name.lower()}%"))
    if description:
        query = query.filter(ArticleState.description.ilike(f"%{description.lower()}%"))
    if refnumber:
        query = query.filter(ArticleState.refnumber.ilike(f"%{refnumber}%"))
    if created_by is not None:
        query = query.filter(ArticleState.created_by == created_by)
    if updated_by is not None:
        query = query.filter(ArticleState.updated_by == updated_by)
    if active is not None:
        query = query.filter(ArticleState.active == active)
    ...
    for field, value, bis_value, operation in [
        (ArticleState.created_at, created_at, created_at_bis, created_at_operation),
        (ArticleState.updated_at, updated_at, updated_at_bis, updated_at_operation),
    ]:
    ...
    query = query.order_by(order_func(getattr(ArticleState, sort_by)))
    ...
    if limit == -1:
        ...
        items = query.filter(ArticleState.active == True).all()
...
def create(db: Session, data: ArticleStateCreate, current_user_id: str = None):
    ...
    existing = db.query(ArticleState).filter(ArticleState.name == data.name).first()
    if existing:
    ...
    try:
        ...
        unique_ref = generate_unique_num_ref(ArticleState, db)
...
def update(db: Session, item_id: str, data: ArticleStateUpdate, current_user_id: str):
    item = db.query(ArticleState).filter(ArticleState.id == item_id).first()
    if not item:
        raise ValueError("L'utilisateur n'a pas été trouvé.")

    # Détection des conflits
    filters = [
        (ArticleState.name == data.name.lower()),
    ]
    existing = db.query(ArticleState).filter(or_(*filters)).first()
...
def delete(db: Session, item_id: str, current_user_id: str):
    item = db.query(ArticleState).filter(ArticleState.id == item_id, ArticleState.active == True).first()
    if not item:
        raise ValueError("ArticleState not found or already deleted.")
    item.active = False
    item.updated_by = current_user_id
    db.commit()
    db.refresh(item)
...
def restore(db: Session, item_id: str, current_user_id: str):
    item = db.query(ArticleState).filter(ArticleState.id == item_id, ArticleState.active == False).first()
    if not item:
        raise ValueError("ArticleState not found or already active.")
    item.active = True
    item.updated_by = current_user_id
    db.commit()
    db.refresh(item)
